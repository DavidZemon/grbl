...
uint8_t gc_execute_line (char *line)
{
  ...
  while (line[char_counter] != 0) { // Loop until no more g-code words in line.
    ...
    switch (letter) {
            ...
            switch (int_value) {
              ...
              case 28:
                switch (mantissa) {
                  ...
                  default:
                    FAIL(
                        STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G28.x command]
                }
              ...
              case 30:
                switch (mantissa) {
                  ...
                  default:
                    FAIL(
                        STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G30.x command]
                }
                mantissa = 0; // Set to zero to indicate valid non-integer G command.
                break;
              case 53:
              ...
              case 92:
                switch (mantissa) {
                  ...
                  default:
                    FAIL(
                        STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G92.x command]
                }
                mantissa = 0; // Set to zero to indicate valid non-integer G command.
                break;
            }
            ...
            switch (int_value) {
              ...
              case 38:
                switch (mantissa) {
                  ...
                  default:
                    FAIL(
                        STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G38.x command]
                }
                mantissa = 0; // Set to zero to indicate valid non-integer G command.
                break;
              case 80:
      ...
      default:
        ...
        if (bit(word_bit) & (bit(WORD_F) | bit(WORD_N) | bit(WORD_P) | bit(
            WORD_T) | bit(WORD_S))) {
          if (value < 0.0) {
  ...
  */
  ...
  if (bit_isfalse(value_words,
                  bit(WORD_S))) {gc_block.values.s = gc_state.spindle_speed;}
  ...
  if (bit_istrue(command_words,
                 bit(MODAL_GROUP_G12))) { // Check if called in block
          ...
          if (value_words & bit(WORD_R)) { // Arc Radius Mode  
            ...
            */
            ...
            if (gc_block.modal.units == UNITS_MODE_INCHES) {
              for (idx = 0; idx < N_AXIS; idx++) { // Axes indices are consistent, so loop may be used to save flash space.
                if (ijk_words & bit(
                    idx)) {gc_block.values.ijk[idx] *= MM_PER_INCH;}
              }
            }
  ...
  bit_false(value_words, (bit(WORD_N) | bit(WORD_F) | bit(WORD_S) | bit(
      WORD_T))); // Remove single-meaning value words.
  if (axis_command) {
    ...
    if (axis_command == AXIS_COMMAND_MOTION_MODE) {
      switch (gc_state.modal.motion) {
        ...
        case MOTION_MODE_CCW_ARC:
          ...
          #else
          mc_arc(gc_state.position, gc_block.values.xyz, gc_block.values.ijk,
                 gc_block.values.r,
                 gc_state.feed_rate, gc_state.modal.feed_rate, axis_0, axis_1,
                 axis_linear);
#endif
          break;
        case MOTION_MODE_PROBE:
      ...
      memcpy(gc_state.position, gc_block.values.xyz,
             sizeof(gc_block.values.xyz)); // gc.position[] = target[];
    }
  }
