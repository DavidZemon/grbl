...
ISR(LIMIT_INT_vect) // DEFAULT: Limit pin change interrupt process.
       {
           ...
           if (sys.state != STATE_ALARM) {
             if (bit_isfalse(sys.execute, EXEC_ALARM)) {
               ...
               bit_true_atomic(sys.execute,
                               (EXEC_ALARM | EXEC_CRIT_EVENT)); // Indicate hard limit critical event
             }
           }
       }
...
void limits_go_home (uint8_t cycle_mask)
{
  ...
  do {
    ...
    if (bit_isfalse(settings.flags,
                    BITFLAG_INVERT_LIMIT_PINS)) {invert_pin = approach;}
    ...
    for (idx = 0; idx < N_AXIS; idx++) {
      ...
      if (bit_istrue(settings.homing_dir_mask,
                     bit(idx))) {target[idx] = -target[idx];}
    ...
    #else
    plan_buffer_line(target, homing_rate,
                     false); // Bypass mc_line(). Directly plan homing motion.
#endif
  ...
  for (idx = 0; idx < N_AXIS; idx++) {
    ...
    if (cycle_mask & bit(idx)) {
      ...
      if (bit_istrue(settings.homing_dir_mask, bit(idx))) {
        ...
        sys.position[idx] = lround(
            settings.max_travel[idx] * settings.steps_per_mm[idx]);
      } else {
        target[idx]       = -settings.homing_pulloff;
        sys.position[idx] = 0;
      }
  ...
  #else
  plan_buffer_line(target, settings.homing_seek_rate,
                   false); // Bypass mc_line(). Directly plan motion.
#endif
...
void limits_soft_check (float *target)
{
  ...
  for (idx = 0; idx < N_AXIS; idx++) {
    ...
    if (soft_limit_error) {
      ...
      bit_true_atomic(sys.execute,
                      (EXEC_ALARM | EXEC_CRIT_EVENT)); // Indicate soft limit critical event
