...
#define AMASS_LEVEL1 (CLKFREQ/8000) // Over-drives ISR (x2). Defined as F_CPU/ (Cutoff frequency in Hz)
#define AMASS_LEVEL2 (CLKFREQ/4000) // Over-drives ISR (x4)
#define AMASS_LEVEL3 (CLKFREQ/2000) // Over-drives ISR (x8)
...
void st_wake_up ()
{
  ...
  if (bit_istrue(settings.flags,
                 BITFLAG_INVERT_ST_ENABLE)) {STEPPERS_DISABLE_PORT |= (1 << STEPPERS_DISABLE_BIT);}
...
void st_go_idle ()
{
  ...
  if (((settings.stepper_idle_lock_time != 0xff) || bit_istrue(sys.execute,
                                                               EXEC_ALARM)) && sys.state != STATE_HOMING) {
  ...
  if (bit_istrue(settings.flags,
                 BITFLAG_INVERT_ST_ENABLE)) {pin_state = !pin_state;} // Apply pin invert.
...
void st_generate_step_dir_invert_masks ()
{
  ...
  for (idx = 0; idx < N_AXIS; idx++) {
    if (bit_istrue(settings.step_invert_mask,
                   bit(idx))) {step_port_invert_mask |= get_step_pin_mask(idx);}
...
void st_prep_buffer ()
{
  while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
          ...
          if (intersect_distance < pl_block->millimeters) { // Either trapezoid or triangle types
            ...
            if (prep.decelerate_after < intersect_distance) { // Trapezoid type
              ...
              prep.maximum_speed    = sqrt(
                  2.0 * pl_block->acceleration * intersect_distance + exit_speed_sqr);
            }
    ...
    */
    ...
    float n_steps_remaining      = ceil(
        steps_remaining); // Round-up current steps remaining
    float last_n_steps_remaining = ceil(
        prep.steps_remaining); // Round-up last steps remaining
