...
void system_init ()
{
  ...
  // What is this doing, and how do I translate it to the Propeller?
//  PINOUT_PCMSK |= PINOUT_MASK;  // Enable specific pins of the Pin Change Interrupt
//  PCICR |= (1 << PINOUT_INT);   // Enable Pin Change Interrupt
}

// TODO: Do this for the Propeller!
    ...
    default :
      ...
      switch (line[char_counter]) {
        ...
        default :  // Storing setting methods [IDLE/ALARM]
          if (!read_float(line, &char_counter,
                          &parameter)) {return (STATUS_BAD_NUMBER_FORMAT);}
          if (line[char_counter++] != '=') {return (STATUS_INVALID_STATEMENT);}
          if (helper_var) { // Store startup line
            ...
            helper_var = gc_execute_line(
                line); // Set helper_var to returned status code.
            if (helper_var) {return (helper_var);}
            else {
              helper_var = trunc(
                  parameter); // Set helper_var to int value of parameter
              settings_store_startup_line(helper_var, line);
            }
          } else { // Store global setting.
            if (!read_float(line, &char_counter,
                            &value)) {return (STATUS_BAD_NUMBER_FORMAT);}
            if (line[char_counter] != 0) {return (STATUS_INVALID_STATEMENT);}
